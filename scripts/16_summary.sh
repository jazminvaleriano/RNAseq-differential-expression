...
FILE_CAGE=/data/users/jvaleriano/lncRNA/06_output_int_analysis/overlap5_CAGE.bed
...
# Add a column to indicate if the transcrips are listed in the results for Intergenic, 3' poly A, 5' CAGE
...
awk -F'\t' 'NR==FNR{a[$4]=$4;next} {print $0 "\t" ($1 in a ? "CAGE_match" : "no_CAGE_match")}' $FILE_CAGE summary.tsv > temp && mv temp summary.tsv

# Add the protein coding potential score
join --nocheck-order -t$'\t' summary.tsv tabbed.pcp.tmp > temp && mv temp summary.tsv

# Add heder row
echo -e "target_id\tchr\tstart\tend\tstrand\tgene_id\tDE_pval\tDE_qval\tlog2fold_change\tintergenic\t3_poly_A_match\t5_CAGE_match\tcoding_potential" | cat - summary.tsv > temp && mv temp summary.tsv
