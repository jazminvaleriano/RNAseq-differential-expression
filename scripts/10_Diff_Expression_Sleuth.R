...
plot_sample_heatmap(so_t, use_filtered = TRUE, color_high = "white",
                    color_low = "dodgerblue", x_axis_angle = 50,
                    annotation_cols = setdiff(colnames(so_t$sample_to_covariates), "sample"),
                    cluster_bool = TRUE)
dev.off()

#Sleuth generates an abundance table in the SO with the sample names and conditions
transcript_abundance_table<-kallisto_table(so_t, use_filtered = FALSE, normalized = TRUE,
...
tr_lrt_significant <- dplyr::filter(tr_lrt_results, qval <= 0.05)
...
# Extracted only protein coding, lncRNA and novel genes
diff_exp_protein_lncRNA_novel <- tr_wt_results %>%
  filter(qval <= 0.05, biotype %in% c("protein_coding","","lncRNA"))
write.csv(tr_wt_significant, file = "diff_exp_protein_lncRNA_novel.csv", row.names = FALSE)
